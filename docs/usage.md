...
## Project Structure
- `src/lpbf`: Main package.
    - `state`: Simulation state container.
    - `config`: Configuration schemas.
    - `physics`: Material models and operators.
    - `scan`: Heat sources and scan path generation.
    - `integrator`: Time stepping logic.
...
# 5. Visualize
from lpbf.viz.static import plot_temperature_field
