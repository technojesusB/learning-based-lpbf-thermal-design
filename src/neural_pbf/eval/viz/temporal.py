...
def peak_tracking_plot(
    ...
    *,
    ...
    ax.plot(steps, T_max_gt, label="GT T_max")
    ax.plot(steps, T_max_pred, label=f"{result.stepper_name} T_max", linestyle="--")
    ax.set_xlabel("Step")
...
def probe_history_plot(
    ...
    *,
    ...
    for probe_name, ix, iy, iz in probes:
        ...
        ax.plot(gt_hist, label=f"{probe_name} GT")
...
def error_evolution_plot(
    ...
    *,
    ...
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(steps, maes, marker="o", markersize=3)
    ax.set_xlabel("Step")
    ax.set_ylabel("MAE [K]")
