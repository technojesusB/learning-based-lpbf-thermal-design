...
class MLflowTracker(ExperimentTracker):
    ...
    def __init__(self, tracking_uri: Optional[str] = None, experiment_name: str = "lpbf", artifact_location: Optional[str] = None):
        ...
        try:
            ...
            if experiment is None:
                mlflow.create_experiment(self.experiment_name, artifact_location=self.artifact_location)
    ...
    def start_run(self, run_name: Optional[str], config: Dict[str, Any], tags: Dict[str, str]):
        ...
        try:
            ...
            if config:
                ...
                # MLflow defines specific max param length, so we might need to be careful with huge configs
                # For now, just log usage.
                # Nested dicts can be logged as JSON artifacts if too large, but here we try simple params.
                flat_params = {} 
                # Very basic flattening could be added here if needed, but let's trust the user or Pydantic serialization
    ...
    def flush(self) -> None:
        # MLflow python client usually handles this, specific implementation might need manual flush
        pass
