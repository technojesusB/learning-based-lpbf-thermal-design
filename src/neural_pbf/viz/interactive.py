...
def figure_temperature_field(state: SimulationState) -> go.Figure:
    ...
    fig = go.Figure(data=go.Heatmap(z=T_slice, colorscale="Inferno", colorbar=dict(title="Temperature [K]")))
...
def figure_cooling_rate(state: SimulationState) -> go.Figure:
    ...
    fig = go.Figure(data=go.Heatmap(z=CR_slice, colorscale="Viridis", colorbar=dict(title="Cooling Rate [K/s]")))

    fig.update_layout(title="Captured Cooling Rate", template="plotly_dark")
    return fig
