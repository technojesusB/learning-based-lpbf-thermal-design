...
def plot_interactive_volume(T: np.ndarray, dx: float, dy: float, dz: float, step: int, max_pts: int = 200000):
...
def plot_interactive_composite(T: np.ndarray, dx: float, dy: float, dz: float, step: int):
    ...
    fig = make_subplots(
        ...
        subplot_titles=(f"3D Volume (Subsampled)", "XY Plane (Top)", "XZ Plane (Side)", "YZ Plane (Front)")
    )
    ...
    fig.add_trace(go.Heatmap(x=x_coords, y=y_coords, z=T[:, :, zi].T, colorscale="Jet", showscale=False), row=1, col=2)
    
    # 3. XZ Heatmap
    z_coords = np.linspace(0, nz*dz*scale, nz)
    fig.add_trace(go.Heatmap(x=x_coords, y=z_coords, z=T[:, yi, :].T, colorscale="Jet", showscale=False), row=2, col=1)
    
    # 4. YZ Heatmap
    fig.add_trace(go.Heatmap(x=y_coords, y=z_coords, z=T[xi, :, :].T, colorscale="Jet", showscale=True), row=2, col=2)
...
def plot_3d_block_mpl(
    ...
    ax.plot_surface(X_side, Y_side, Z_side, facecolors=m.to_rgba(T[:, 0, :].T), shade=False)
    ...
    ax.plot_surface(X_right, Y_right, Z_right, facecolors=m.to_rgba(T[-1, :, :].T), shade=False)
...
def plot_cross_sections(
    ...
    plot_surface_heatmap_mpl(
        ...
        unit=unit, vmin=vmin, vmax=vmax, cmap=cmap
    )
...
def plot_composite_thermal_view(
    ...
    plot_3d_block_mpl_ax(ax_3d, T, dx, dy, dz, unit=unit, title=f"3D Field - Step {step}")
...
def plot_3d_block_mpl_ax(
    ax, T, dx, dy, dz, 
    vmin=None, vmax=None, cmap='jet', unit='mm', title=None
):
    ...
    if nx > 200 or ny > 200: stride = 2 # Adaptive stride for performance
    
    # Top
    X, Y = np.meshgrid(np.linspace(0, Lx, nx), np.linspace(0, Ly, ny))
    ax.plot_surface(X, Y, np.full_like(X, Lz), facecolors=m.to_rgba(T[..., -1].T), shade=False, rstride=stride, cstride=stride)
    
    # Side (Y=0)
    X_s, Z_s = np.meshgrid(np.linspace(0, Lx, nx), np.linspace(0, Lz, nz))
    ax.plot_surface(X_s, np.zeros_like(X_s), Z_s, facecolors=m.to_rgba(T[:, 0, :].T), shade=False, rstride=stride, cstride=stride)
    
    # Front (X=Lx)
    Y_f, Z_f = np.meshgrid(np.linspace(0, Ly, ny), np.linspace(0, Lz, nz))
    ax.plot_surface(np.full_like(Y_f, Lx), Y_f, Z_f, facecolors=m.to_rgba(T[-1, :, :].T), shade=False, rstride=stride, cstride=stride)
