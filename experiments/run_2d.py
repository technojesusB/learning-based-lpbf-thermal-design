...
def run_2d_experiment(
    ...
    if art_cfg is None:
        art_cfg = ArtifactConfig(
            enabled=True, png_every_n_steps=50, html_every_n_steps=100, make_report=True
        )
    ...
    T = torch.full(
        (1, 1, sim_cfg.Nx, sim_cfg.Ny), 300.0, device=device, dtype=torch.float32
    )
    # Mask: 0=Powder
    ...
    try:
        for i in range(steps):
            ...
            Q_flux = source.intensity(
                X, Y, None, x0=float(current_pos[0]), y0=float(current_pos[1])
            )

            # Step (Adaptive)
            # Q_flux is (Nx, Ny), need (1, 1, Nx, Ny)
            state = stepper.step_adaptive(
                state, dt_target=sim_cfg.dt_base, Q_ext=Q_flux.unsqueeze(0).unsqueeze(0)
            )
