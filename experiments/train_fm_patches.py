"""train_fm_patches.py — UNet (VelocityNet) baseline for LPBF FM surrogate.

Entry point for the v1 baseline. Core dataset, training, and evaluation
utilities live in neural_pbf.*; this script only contains main(), argument
parsing, and training batch logic.
"""
...
def main() -> None:
    ...
    # --- batch-level training logic (UNet-specific, pure FM loss) ---
    ...
    # --- per-epoch callbacks (closures over device, model, split) ---
    ...
    with tracker.start_run(
        ...
        config={"epochs": args.epochs, "batch_size": args.batch_size,
                "lr": args.lr, "n_train": len(split.train_ds)},
        ...
        history: TrainHistory = run_train_loop(
            ...
            epochs=args.epochs, val_every=1,
        )
            ...
            def _test_rollout(batch: dict[str, Any]) -> tuple[Tensor, Tensor]:
                with torch.no_grad():
                    ...
                    for i in range(args.test_n_steps):
                        tau = torch.full((T_tgt.shape[0],), i * dt, device=device)
                        x = x + model(torch.cat([x, mask, Q], dim=1), tau, cond_emb) * dt
