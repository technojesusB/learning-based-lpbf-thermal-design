...
def run_benchmark(nx=512, ny=256, nz=128, steps=50, material="default", use_triton=False):
    ...
    sim_cfg = SimulationConfig(
        Lx=nx*1e-2, Ly=ny*1e-2, Lz=nz*1e-2, # mm
        Nx=nx, Ny=ny, Nz=nz,
        dt_base=2e-6,
    ...
    with tracker:
        for i in range(steps):
            state = stepper.step_explicit_euler(state, sim_cfg.dt_base, Q_ext=Q_ext, use_triton=use_triton)
...
if __name__ == "__main__":
    ...
    res_tdep = run_benchmark(nx=NX, ny=NY, nz=NZ, steps=STEPS, material="ss316l", use_triton=False)
    
    # Case 3: Triton (SS316L LUT)
    res_triton = run_benchmark(nx=NX, ny=NY, nz=NZ, steps=STEPS, material="ss316l", use_triton=True)
    
    print("\n" + "="*40)
    print(f"SUMMARY ({NX}x{NY}x{NZ} Grid)")
    print(f"PyTorch (Const):   {res_base.elapsed_ms/STEPS:.2f} ms/step, {res_base.max_vram_mb:.1f} MB")
    print(f"PyTorch (SS316L): {res_tdep.elapsed_ms/STEPS:.2f} ms/step, {res_tdep.max_vram_mb:.1f} MB")
    print(f"Triton  (SS316L): {res_triton.elapsed_ms/STEPS:.2f} ms/step, {res_triton.max_vram_mb:.1f} MB")
